...
def sample_card(row: dict[str, Any], index: int, kind: str) -> str:
...
<article class="sample {esc(kind)}">
  <div class="sample-top"><strong>#{index} · {esc(as_text(row.get('topic')))}</strong><span>{label_tag(relevance)} {label_tag(grade)}</span></div>
...
def main() -> None:
    ...
    for topic, topic_items in sorted(
        ...
        topic_rows.append(
            [
                Html(f'<span class="topic">{esc(topic)}</span>'),
                fmt_number(len(topic_items)),
                fmt_number(success),
                fmt_number(selected),
                pct(selected, success),
                fmt_number(unresolved),
            ]
        )
...
.callout-list {{ display:grid; gap:9px; }} .table-wrap {{ overflow:auto; border:1px solid var(--line); border-radius:8px; }} table {{ width:100%; border-collapse:collapse; }} th,td {{ padding:9px 10px; border-bottom:1px solid var(--line); text-align:left; vertical-align:top; font-size:13px; }} th {{ background:#edf2f5; color:#3e4b57; white-space:nowrap; }} tr:last-child td {{ border-bottom:0; }} .num {{ text-align:right; font-variant-numeric:tabular-nums; }} .topic {{ display:block; min-width:300px; max-width:520px; }}
...
<section><div class="metrics">
  ...
  {metric_card('未解决', fmt_number(len(unresolved_rows)), f'占全部线程 {pct(len(unresolved_rows), total)}', status_tone)}
</div></section>

<section><div class="section-head"><div><h2>结论与口径</h2><p class="hint">报告只使用最新三字段工作簿，不重新引入立场字段。</p></div></div>
<div class="callout-list">
  <div class="callout {'bad' if unresolved_rows else 'good'}"><strong>运行完整性：</strong>{fmt_number(len(completed_rows))} 条有效标注，{fmt_number(len(unresolved_rows))} 条仍为空；因此当前报告是复核版，不是最终封版。</div>
  <div class="callout"><strong>保留规则：</strong><code>topic_relevance ∈ {{strongly_relevant, relevant}}</code> 且 <code>training_grade != unusable</code>。共保留 {fmt_number(len(selected_rows))} 个线程。</div>
  <div class="callout warn"><strong>人工复核：</strong><code>relevant_context_only</code> 和 <code>borderline_sample</code> 按当前宽松口径保留，但应优先进入人工审查。</div>
...
<section><div class="section-head"><div><h2>未解决错误</h2><p class="hint">来自 errors.csv；这些行没有进入当前保留集合。</p></div></div>{render_table(['错误代码','数量','占未解决'], error_table_rows, {1})}</section>

<section><div class="section-head"><div><h2>按 Topic 汇总</h2><p class="hint">保留率以该 Topic 成功标注行作为分母。</p></div></div>{render_table(['Topic','总线程','成功','保留','成功口径保留率','未解决'], topic_rows, {1,2,3,5})}</section>

<section><div class="section-head"><div><h2>可用线程固定抽样</h2><p class="hint">跨 Topic 固定抽取 {len(usable_samples)} 条；随机种子 {args.seed}。</p></div></div><div class="samples">{sample_html}</div></section>
<section><div class="section-head"><div><h2>排除线程固定抽样</h2><p class="hint">从成功但未通过保留规则的线程中固定抽取 {len(unusable_samples)} 条。</p></div></div><div class="samples">{unusable_html}</div></section>
<section><div class="section-head"><div><h2>未解决线程固定抽样</h2><p class="hint">这些线程保留原始帖子和评论内容，但标注字段为空。</p></div></div><div class="samples">{unresolved_html}</div></section>

<section class="footer">报告由 <code>scripts/generate_facebook_comment_report.py</code> 生成。源数据：<code>{esc(args.workbook)}</code>；错误：<code>{esc(args.errors_csv)}</code>；下游评论者：<code>{esc(args.crawl_csv)}</code>。</section>
</main>
</body>
</html>
"""
