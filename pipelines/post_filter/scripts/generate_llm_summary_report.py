...
BENCHMARK_ROOT = ROOT.parents[2]
DEFAULT_MESSAGES = BENCHMARK_ROOT / "socialmedia_data/clean_outputs/messages_clean.jsonl"
DEFAULT_COMMENT_SUMMARY = ROOT / "output/qwen32b_8gpu/merged/comment_match_summary.csv"
DEFAULT_COMMENT_TRANSLATIONS = ROOT / "output/qwen32b_8gpu/merged/comment_match_translations.json"
DEFAULT_COMMENT_TRANSLATION_OVERRIDES = (
    ROOT / "output/qwen32b_8gpu/merged/comment_match_cloud_translation_overrides.json"
)
AUDIT_SEED = 20260714
FOCUS_SEED = 20260715
COMMENT_AUDIT_SEED = 20260716
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="Cleaned messages JSONL containing posts/videos/comments.",
    )
    parser.add_argument(
        "--comment-summary",
        type=Path,
        default=DEFAULT_COMMENT_SUMMARY,
        help="Machine-readable platform comment-match summary CSV.",
    )
    parser.add_argument(
        "--comment-translations",
        type=Path,
        default=DEFAULT_COMMENT_TRANSLATIONS,
        help="JSON cache containing Chinese translations for sampled posts and comments.",
    )
    parser.add_argument(
        "--comment-translation-overrides",
        type=Path,
        default=DEFAULT_COMMENT_TRANSLATION_OVERRIDES,
        help="Cloud-reviewed translations merged over the local comment translation cache.",
    )
    parser.add_argument(
        "--comment-audit-per-platform",
        type=int,
        default=12,
        help="Include all matched comments from this many distinct retained posts per platform.",
    )
    parser.add_argument(
        "--finalized",
        action="store_true",
...
def normalise_relation_url(value: str) -> str:
    """Normalise a post URL only for joining comments to retained roots."""
...
def build_comment_report(
    ...
    messages_path: Path,
    audit_per_platform: int,
    translations: dict[str, Any],
    require_translations: bool = True,
) -> dict[str, Any]:
    """Join cleaned comments to unique retained roots and build QA artifacts."""

    if audit_per_platform < 0:
        raise SystemExit("--comment-audit-per-platform must be >= 0")
    kept_rows = [row for row in rows if is_success(row) and is_kept(row)]
    pair_counts = Counter(row.get("platform", "") or "unknown" for row in kept_rows)
    roots: dict[str, dict[str, Any]] = {}
    for row in kept_rows:
        mid = str(row.get("mid") or row.get("record_id") or "").strip()
        if not mid:
            continue
        root = roots.setdefault(
            mid,
            {
                ...
                "url": str(row.get("url", "")),
                "post_text": str(row.get("cont_clean", "")),
                "topics": set(),
            },
        )
        topic = str(row.get("topic", "")).strip()
        if topic:
            root["topics"].add(topic)
    ...
    for mid, root in roots.items():
        ...
        _add_unique_lookup(
            ...
            ambiguous_url,
            platform=root["platform"],
            value=normalise_relation_url(root["url"]),
            mid=mid,
        )

    all_comments = Counter()
    source_root_keys: dict[str, set[str]] = defaultdict(set)
    matched: dict[str, dict[str, dict[str, Any]]] = defaultdict(dict)
    with messages_path.open(encoding="utf-8") as handle:
        for line in handle:
            ...
            all_comments[platform] += 1
            r_mid = str(record.get("r_mid", "")).strip()
            f_mid = str(record.get("f_mid", "")).strip()
            r_url = normalise_relation_url(str(record.get("r_url", "")))
            f_url = normalise_relation_url(str(record.get("f_url", "")))
            source_key = r_mid or r_url or f_mid or f_url
            if source_key:
                source_root_keys[platform].add(source_key)
            candidates = (
            ...
            resolved_mid = ""
            method = ""
            for candidate_method, candidate_mid in candidates:
                if candidate_mid:
                    resolved_mid = candidate_mid
                    method = candidate_method
                    break
            if not resolved_mid:
                continue
            comment_mid = str(record.get("mid") or record.get("record_id") or "").strip()
            if not comment_mid:
                continue
            matched[resolved_mid][comment_mid] = {
                "comment_mid": comment_mid,
                "comment_text": str(record.get("cont_clean") or record.get("cont") or ""),
                "comment_timestamp": str(record.get("pt_iso_utc", "")),
                "comment_likes": str(record.get("nlike", "")),
                "relation_method": method,
            }

    link_methods = Counter(
        (roots[mid]["platform"], comment["relation_method"])
        for mid, comments in matched.items()
        for comment in comments.values()
    )

    platforms = sorted(set(pair_counts) | set(all_comments))
    platform_rows: list[dict[str, Any]] = []
    for platform in platforms:
        platform_roots = [mid for mid, root in roots.items() if root["platform"] == platform]
        matched_roots = [mid for mid in platform_roots if matched.get(mid)]
        counts = [len(matched[mid]) for mid in matched_roots]
        method_text = " / ".join(
            f"{method}:{count:,}"
            for (method_platform, method), count in sorted(link_methods.items())
            if method_platform == platform
        )
        platform_rows.append(
            {
                "platform": platform,
                "retained_pairs": pair_counts[platform],
                "retained_unique_posts": len(platform_roots),
                "all_clean_comments": all_comments[platform],
                "distinct_comment_roots": len(source_root_keys[platform]),
                "matched_retained_posts": len(matched_roots),
                "unmatched_retained_posts": len(platform_roots) - len(matched_roots),
                "post_coverage_percent": (
                    100 * len(matched_roots) / len(platform_roots) if platform_roots else 0.0
                ),
                "linked_comments": sum(counts),
                "linked_share_percent": (
                    100 * sum(counts) / all_comments[platform] if all_comments[platform] else 0.0
                ),
                "comments_p50": percentile(counts, 0.5),
                "comments_p90": percentile(counts, 0.9),
                "comments_max": max(counts, default=0),
                "relation_methods": method_text,
            }
        )

    post_translations = translations.get("posts", {})
    comment_translations = translations.get("comments", {})
    if not isinstance(post_translations, dict) or not isinstance(comment_translations, dict):
        raise SystemExit("Comment translations JSON must contain object-valued posts and comments.")
    missing_post_translations: list[str] = []
    missing_comment_translations: list[str] = []
    audit_posts: list[dict[str, Any]] = []
    ...
    for platform_row in platform_rows:
        ...
        candidate_roots = sorted(
            mid
            for mid, root in roots.items()
            if root["platform"] == platform and matched.get(mid)
        )
        if not candidate_roots or audit_per_platform == 0:
            continue
        generator = random.Random(f"{COMMENT_AUDIT_SEED}:{platform}")
        selected_roots = generator.sample(
            candidate_roots, min(audit_per_platform, len(candidate_roots))
        )
        for mid in selected_roots:
            root = roots[mid]
            post_text_zh = str(post_translations.get(mid, "")).strip()
            if not post_text_zh:
                missing_post_translations.append(mid)
            comments = []
            for original_comment in sorted(
                matched[mid].values(), key=lambda item: item["comment_mid"]
            ):
                comment = dict(original_comment)
                comment_mid = comment["comment_mid"]
                comment["comment_text_zh"] = str(comment_translations.get(comment_mid, "")).strip()
                if not comment["comment_text_zh"]:
                    missing_comment_translations.append(comment_mid)
                comments.append(comment)
            topics = " || ".join(sorted(root["topics"]))
            audit_posts.append(
                {
                    "platform": platform,
                    "topics": topics,
                ...
                audit_rows.append(
                    {
                        ...
                        "comment_text_zh": comment["comment_text_zh"],
                        "comment_timestamp": comment["comment_timestamp"],
                        "comment_likes": comment["comment_likes"],
                        "comments_on_matched_post": str(len(comments)),
                        "relation_method": comment["relation_method"],
                        "human_match_correct": "",
                        "human_comment_relevance": "",
                        "human_stance_label": "",
                        "human_note": "",
                    }
                )

    if require_translations and (missing_post_translations or missing_comment_translations):
        raise SystemExit(
            "Missing sampled comment translations: "
            f"posts={len(set(missing_post_translations))}, "
            f"comments={len(set(missing_comment_translations))}. "
            "Run scripts/generate_comment_translations.py first."
        )

    total_unique_posts = sum(row["retained_unique_posts"] for row in platform_rows)
    total_matched_posts = sum(row["matched_retained_posts"] for row in platform_rows)
    total_comments = sum(row["all_clean_comments"] for row in platform_rows)
    total_linked = sum(row["linked_comments"] for row in platform_rows)
    return {
        "messages_path": messages_path,
        "platform_rows": platform_rows,
        "audit_posts": audit_posts,
        "audit_rows": audit_rows,
        "totals": {
            "comment_platforms": sum(row["all_clean_comments"] > 0 for row in platform_rows),
            "retained_unique_posts": total_unique_posts,
            "matched_retained_posts": total_matched_posts,
            "all_clean_comments": total_comments,
            "linked_comments": total_linked,
        },
...
def comment_post_card(row: dict[str, Any], index: int) -> str:
    comments = row["comments"]
    relation_methods = " / ".join(sorted({comment["relation_method"] for comment in comments}))
    meta = (
        f"帖子 #{index} / {row.get('platform', '')} / relation: {relation_methods} / "
        f"完整展示 {len(comments)} 条已匹配评论"
    )
    link = ""
    if row.get("post_url", "").startswith(("https://", "http://")):
        link = (
            '<div class="sample-link"><a href="'
            + esc(row["post_url"])
            + '" target="_blank" rel="noopener noreferrer">打开原始帖子</a></div>'
        )
    comment_items = "".join(
        '<li class="comment-item">'
        f'<div class="comment-meta">评论 {comment_index} / 时间: '
        f'{esc(comment.get("comment_timestamp", "") or "未记录")} / '
        f'likes: {esc(comment.get("comment_likes", "") or "未记录")} / '
        f'ID: {esc(comment.get("comment_mid", ""))}</div>'
        f'<div class="comment-text"><strong>原文：</strong>{esc(comment.get("comment_text", ""))}</div>'
        f'<div class="comment-text translation"><strong>中文：</strong>{esc(comment.get("comment_text_zh", ""))}</div>'
        "</li>"
    ...
    return (
        '<article class="sample">'
        f'<div class="sample-meta">{esc(meta)}</div>'
        f'<div class="sample-text"><strong>Topic：</strong>{esc(row.get("topics", ""))}</div>'
        f'<div class="sample-text"><strong>帖子原文：</strong>{esc(row.get("post_text", ""))}</div>'
        f'<div class="sample-text translation"><strong>帖子中文：</strong>{esc(row.get("post_text_zh", ""))}</div>'
        f"{link}<ol class=\"comment-list\">{comment_items}</ol></article>"
    )
...
def render_report(
    ...
    comment_platform_rows = comment_report["platform_rows"]
    comment_totals = comment_report["totals"]
    comment_match_table = table(
        [
            "平台",
            "保留 pair",
            "唯一保留帖",
        ...
        [
            [
                esc(item["platform"]),
                fmt_number(item["retained_pairs"]),
                fmt_number(item["retained_unique_posts"]),
                fmt_number(item["all_clean_comments"]),
                fmt_number(item["matched_retained_posts"]),
                f"{item['post_coverage_percent']:.1f}%" if item["retained_unique_posts"] else "-",
                fmt_number(item["linked_comments"]),
                f"{item['linked_share_percent']:.1f}%" if item["all_clean_comments"] else "-",
                f"{item['comments_p50']} / {item['comments_p90']} / {item['comments_max']}",
                esc(item["relation_methods"] or "无可用评论"),
            ]
            for item in comment_platform_rows
        ],
        {1, 2, 3, 4, 6},
    )
    comment_audit_posts = comment_report["audit_posts"]
    comment_audit_rows = comment_report["audit_rows"]
    translation_review_note = str(
        comment_report.get("translation_review_note")
        or "中文译文仅用于辅助阅读；语义判断应以原文为准。"
    )
    translation_override_name = str(
        comment_report.get("translation_override_name")
        or DEFAULT_COMMENT_TRANSLATION_OVERRIDES.name
    )
    comment_audit_cards = "".join(
        comment_post_card(row, index) for index, row in enumerate(comment_audit_posts, 1)
    )
...
.samples,.sample-width {{ width:100%; }}
...
.sample {{ min-width:0; padding:13px; }} .sample-meta {{ margin-bottom:6px; color:var(--muted); font-size:12px; overflow-wrap:anywhere; }} .sample-text {{ margin:6px 0; font-size:13px; white-space:pre-wrap; overflow-wrap:anywhere; }} .sample-link {{ margin-top:9px; font-size:13px; overflow-wrap:anywhere; }} a {{ color:#0b5f7e; }}
.comment-list {{ display:grid; gap:8px; margin:12px 0 0; padding:0; list-style:none; }}
.comment-item {{ padding:10px 11px; border:1px solid var(--line); border-radius:7px; background:#f8fafc; }}
.comment-meta {{ margin-bottom:5px; color:var(--muted); font-size:11px; overflow-wrap:anywhere; }}
.comment-text {{ font-size:13px; white-space:pre-wrap; overflow-wrap:anywhere; }}
.translation {{ margin-top:6px; padding-top:6px; border-top:1px dashed var(--line); color:#214f3b; }}
...
<p class="subtitle">筛选结果：{esc(pairs_path)}。评论数据：{esc(comment_report['messages_path'])}。生成时间：{esc(now)}。</p>
...
<section><div class="section-head"><div><h2>保留帖子与现有评论匹配</h2><p class="hint">按唯一帖子统计，避免同一帖子命中多个 topic 后重复计算评论。评论来自清洗数据快照，不代表平台完整评论区。</p></div></div>
<div class="cards">
{metric_card("有评论数据的平台", fmt_number(comment_totals["comment_platforms"]), "当前清洗文件中为 Instagram 与 YouTube", "", None)}
{metric_card("匹配到评论的保留帖", fmt_number(comment_totals["matched_retained_posts"]), f"占唯一保留帖 {pct(comment_totals['matched_retained_posts'], comment_totals['retained_unique_posts'])}", "good", pct_value(comment_totals["matched_retained_posts"], comment_totals["retained_unique_posts"]))}
{metric_card("成功关联评论", fmt_number(comment_totals["linked_comments"]), f"占全部清洗评论 {pct(comment_totals['linked_comments'], comment_totals['all_clean_comments'])}", "warn", pct_value(comment_totals["linked_comments"], comment_totals["all_clean_comments"]))}
{metric_card("清洗评论总数", fmt_number(comment_totals["all_clean_comments"]), "包含尚未匹配到最终保留帖的评论", "", None)}
</div>
<div style="margin-top:12px">{comment_match_table}</div>
<div class="callout-list" style="margin-top:12px">
<div class="callout"><strong>匹配口径</strong>YouTube 优先使用原生视频 ID；Instagram 使用标准化帖子 URL。统计只计算最终保留帖下能够确定父帖的唯一清洗评论。</div>
<div class="callout warn"><strong>尚未进行 comment-level 语义筛选</strong>这些评论只是“挂在保留帖子下”，并不保证评论自身与 topic 相关，也尚未经过人工 stance 标注。</div>
<div class="callout"><strong>关系键审计</strong>冲突 native key：{fmt_number(comment_report['ambiguous_native_keys'])}；冲突 URL key：{fmt_number(comment_report['ambiguous_url_keys'])}。机器可读统计写入 <code>comment_match_summary.csv</code>。</div>
</div></section>

<section><div class="section-head"><div><h2>匹配评论摘录</h2><p class="hint">按平台固定抽取 {len(comment_audit_posts)} 个不同保留帖子，并完整展示这些帖子下的全部 {len(comment_audit_rows)} 条已匹配评论；帖子与评论均同时提供原文和中文译文。随机种子 {COMMENT_AUDIT_SEED}。</p></div></div><div class="callout good sample-width" style="margin-bottom:12px"><strong>云端翻译说明</strong>{esc(translation_review_note)} 译文用于辅助阅读，语义判断仍以原文为准。</div><div class="samples">{comment_audit_cards}</div></section>
...
<p class="footer">报告由 <code>scripts/generate_llm_summary_report.py</code> 生成；评论平台统计写入 <code>comment_match_summary.csv</code>，本地译文基础缓存为 <code>comment_match_translations.json</code>，当前报告使用云端复核覆盖层 <code>{esc(translation_override_name)}</code>；当前累计 tokens：{fmt_number(total_tokens)}。</p>
</main></body></html>"""
...
def main() -> None:
    ...
    if not args.comment_translations.exists():
        raise SystemExit(f"Comment translations JSON not found: {args.comment_translations}")
    if not args.comment_translation_overrides.exists():
        raise SystemExit(
            "Comment translation overrides JSON not found: "
            f"{args.comment_translation_overrides}"
        )
    ...
    comment_translations = json.loads(args.comment_translations.read_text(encoding="utf-8"))
    if not isinstance(comment_translations, dict):
        raise SystemExit("Comment translations JSON must be an object.")
    comment_translation_overrides = json.loads(
        args.comment_translation_overrides.read_text(encoding="utf-8")
    )
    if not isinstance(comment_translation_overrides, dict):
        raise SystemExit("Comment translation overrides JSON must be an object.")
    for section in ("posts", "comments"):
        base_values = comment_translations.setdefault(section, {})
        override_values = comment_translation_overrides.get(section, {})
        if not isinstance(base_values, dict) or not isinstance(override_values, dict):
            raise SystemExit(
                f"Comment translation JSON section {section!r} must be an object."
            )
        base_values.update(override_values)
    comment_report = build_comment_report(
        rows,
        args.messages,
        args.comment_audit_per_platform,
        comment_translations,
    )
    comment_report["translation_provider"] = str(
        comment_translation_overrides.get("provider", "cloud-reviewed")
    )
    comment_report["translation_review_note"] = str(
        comment_translation_overrides.get("review_note", "")
    )
    comment_report["translation_override_name"] = args.comment_translation_overrides.name
    write_dict_csv(
        args.comment_summary,
        comment_report["platform_rows"],
        [
            "platform",
            "retained_pairs",
            "retained_unique_posts",
            "all_clean_comments",
            "distinct_comment_roots",
            "matched_retained_posts",
            "unmatched_retained_posts",
            "post_coverage_percent",
            "linked_comments",
            "linked_share_percent",
            "comments_p50",
    ...
    totals = comment_report["totals"]
    print(f"Wrote {args.output} from {len(rows):,} post-topic rows")
    print(
        "Matched "
        f"{totals['linked_comments']:,} comments to "
        f"{totals['matched_retained_posts']:,} of {totals['retained_unique_posts']:,} retained posts"
    )
    print(f"Wrote {args.comment_summary}")
    print(
        f"Embedded {len(comment_report['audit_rows']):,} translated comments "
        f"from {len(comment_report['audit_posts']):,} sampled posts in the HTML report"
    )
