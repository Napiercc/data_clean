...
for shard in $(seq 0 $((NUM_SHARDS - 1))); do
  ...
  echo "Starting sample shard ${shard}/${NUM_SHARDS} on port ${port}; log: ${log_file}"
...
echo "$(date '+%Y-%m-%d %H:%M:%S') progress: ${done_rows}/${TOTAL_ROWS} (${final_percent}%), sample shard jobs finished"
